...
def _livekit_agent_dispatch_name() -> str:
    """Must match ``agent_name`` on the Python worker (``LIVEKIT_AGENT_NAME`` or empty)."""
    return (os.getenv("LIVEKIT_AGENT_NAME", "") or "").strip()
...
def get_token(
    ...
    # Dispatch the kid-tutor agent when the child joins (otherwise the worker may sit idle
    # and you will never see Cartesia / BitHuman logs in agent.py).
    agent_name = _livekit_agent_dispatch_name()
