...
| `OPENAI_API_KEY`      | Yes             | Chat completions only (`OPENAI_LLM_MODEL`); not used for speech |
| `CARTESIA_API_KEY`    | Yes             | Ink STT + Sonic TTS for the tutor pipeline |
| `OPENAI_LLM_MODEL`    | No              | Default `gpt-4.1-mini` — must be a **chat** model, not `gpt-realtime` |
...
**No `bithuman-agent` lines in the terminal (only `livekit.agents`)**  
Job subprocesses sometimes start without a log handler. Restart the worker after updating `agent.py` (it adds a stderr fallback). Ensure `LIVEKIT_LOG_LEVEL` / `LOG_LEVEL` is not `ERROR` if you want INFO. Run unbuffered: `python -u agent.py dev`.

**No tutor speech (Cartesia connects, STT works, but silence)**  
Check agent logs for `failed to generate LLM completion` and `401` / `model.request`. Your `OPENAI_API_KEY` must allow **Chat Completions** for `OPENAI_LLM_MODEL` (e.g. `gpt-4.1-mini`). Restricted keys need the `model.request` scope; a Realtime-only or read-only project role will not work for this pipeline.
