...
def looks_like_explicit_readiness_reply(transcript: str) -> bool:
    """True only for clear \"let's start / I'm ready\" phrasing.

    Used **before** the lesson has started (greeting phase). Do not use the broad
    filler-token heuristic from ``looks_like_readiness_acknowledgment`` here: answers
    like \"fine\", \"good\", or \"great\" to \"how are you?\" are all filler tokens
    but must still trigger the greeting → lesson handoff.
    """
...
def score_utterance(expected: str, transcript: str, thresholds: dict[str, Any]) -> dict[str, Any]:
    ...
    # Handle letter-by-letter spelling: "C A T" → concatenate to "cat" and score that.
    norm_singles = [_normalize_word(t) for t in raw_tokens if _normalize_word(t)]
    if norm_singles and all(len(w) == 1 for w in norm_singles):
        spelled = "".join(norm_singles)
        dist = _levenshtein(exp, spelled)
        mlen = max(len(exp), len(spelled), 1)
