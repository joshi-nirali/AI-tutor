...
/**
 * Worker identity is `agent-…` (LiveKit agent worker). bitHuman joins as `bithuman-avatar-agent`.
...
function PublishChildProfile({ topicSlug, childName, tutorSlug, cartesiaVoiceId }) {
    ...
    const publish = () => {
      ...
      const payload = enc.encode(
        JSON.stringify({
          ...
          topicSlug,
          childName: raw,
          tutorSlug: (tutorSlug || "").toLowerCase(),
          cartesiaVoiceId: (cartesiaVoiceId || "").trim(),
        })
      );
  ...
  }, [room, room.state, topicSlug, childName, tutorSlug, cartesiaVoiceId]);

  return null;
}

/**
 * Play Cartesia TTS from the lesson agent participant only (`agent-…`).
 * BitHuman's participant carries lip-sync video and a different cloud voice — ignore its audio.
 */
function TutorAgentAudioRenderer() {
  const room = useMaybeRoomContext();
  ...
  useEffect(() => {
    ...
    const onAudioPlaybackStatusChanged = (playing) => {
      if (!playing) return;
      elementsRef.current.forEach(tryPlay);
    };
    ...
    room.remoteParticipants.forEach((p) => {
      p.audioTrackPublications.forEach((pub) => {
        if (pub.track) attachAgentAudio(pub.track, p);
      });
    });
 ...
 * Content stays mounted; overlay uses pointer-events: none so taps reach session UI under it.
 */
