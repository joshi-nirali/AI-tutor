...
| `CARTESIA_API_KEY` | Cartesia Ink STT + Sonic TTS (all tutor speech) |
| `OPENAI_LLM_MODEL` | OpenAI **chat** model for reasoning (not a Realtime speech model) |
...
| No tutor audio | Check `CARTESIA_API_KEY` and agent logs; child should hear the `agent-*` track (BitHuman participant audio is muted in the UI) |
